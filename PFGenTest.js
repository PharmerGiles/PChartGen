...
const { NamedStyle, Font, Border, Side, DEFAULT_FONT } = require('exceljs');
const { ColumnDimension, DimensionHolder } = require('exceljs');
const { getColumnLetter } = require('exceljs/lib/utils/colCache');
...
const ExcelExport = () => {
    ...
    const imgPath = path.join(__dirname, 'hhft-logo.jpg');
    ...
    const borderLBr = { top: borderStyles.thin, bottom: borderStyles.thin, left: borderStyles.thin };
    const borderMid = { top: borderStyles.thin, bottom: borderStyles.thin };
    const borderRBr = { top: borderStyles.thin, bottom: borderStyles.thin, right: borderStyles.thin };
    const borderTop = { top: borderStyles.thin };
    const borderBtm = { bottom: borderStyles.thin };
    const borderL = { left: borderStyles.thin };
    const borderR = { right: borderStyles.thin };
    const borderTLC = { top: borderStyles.thin, left: borderStyles.thin };
    const borderTRC = { top: borderStyles.thin, right: borderStyles.thin };
    const borderBLC = { bottom: borderStyles.thin, left: borderStyles.thin };
    const borderBRC = { bottom: borderStyles.thin, right: borderStyles.thin };
...
function App() {
  ...
  return createElement('div', null,
    ...
    createElement('input', { type: 'text', value: firstDoseDays, onChange: e => setFirstDoseDays(e.target.value) }),
    createElement('label', null, 'What is the initial dose prescribed (in mg)?'),
    createElement('input', { type: 'text', value: initDose, onChange: e => setInitDose(e.target.value) }),
    createElement('label', null, 'Remaining dose prescribed after initial stepdown (in mg?)'),
    createElement('input', { type: 'text', value: remDose, onChange: e => setRemDose(e.target.value) }),
    createElement('label', null, 'How many days required for each dose level?'),
    createElement('input', { type: 'text', value: days, onChange: e => setDays(e.target.value) }),
    createElement('label', null, 'How much is dose to be reduced by (in mg)?'),
    createElement('input', { type: 'text', value: doseRed, onChange: e => setDoseRed(e.target.value) }),
    createElement('label', null, 'What are the final instructions e.g. ("then STOP.")?'),
    createElement('input', { type: 'text', value: finalInst, onChange: e => setFinalInst(e.target.value) }),
    createElement('label', null, "What is the patient's name?"),
    createElement('input', { type: 'text', value: ptName, onChange: e => setPtName(e.target.value) }),
    createElement('button', { onClick: handleExport }, 'Export'),
    createElement('label', null, 'You need '),
    createElement('label', { style: { width: '10px' } }, result),
